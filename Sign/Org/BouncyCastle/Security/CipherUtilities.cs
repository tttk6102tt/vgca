...
namespace Sign.Org.BouncyCastle.Security
{
    public sealed class CipherUtilities
    {
        ...
        static CipherUtilities()
        {
            ...
            ((CipherAlgorithm)(object)Sign.Org.BouncyCastle.Utilities.Enums.GetArbitraryValue(typeof(CipherAlgorithm))).ToString();
            ((CipherMode)(object)Sign.Org.BouncyCastle.Utilities.Enums.GetArbitraryValue(typeof(CipherMode))).ToString();
            ((CipherPadding)(object)Sign.Org.BouncyCastle.Utilities.Enums.GetArbitraryValue(typeof(CipherPadding))).ToString();
        ...
        public static IBufferedCipher GetCipher(string algorithm)
        {
            ...
            try
            {
                cipherAlgorithm = (CipherAlgorithm)(object)Sign.Org.BouncyCastle.Utilities.Enums.GetEnumValue(typeof(CipherAlgorithm), text2);
            }
            ...
            if (array.Length > 2)
            {
                ...
                else
                {
                    try
                    {
                        cipherPadding = (CipherPadding)(object)Sign.Org.BouncyCastle.Utilities.Enums.GetEnumValue(typeof(CipherPadding), text4);
                    }
                    catch (ArgumentException)
                    {
                        throw new SecurityUtilityException("Cipher " + algorithm + " not recognised.");
            ...
            if (array.Length > 1)
            {
                ...
                try
                {
                    switch ((text6 == "") ? CipherMode.NONE : ((CipherMode)(object)Sign.Org.BouncyCastle.Utilities.Enums.GetEnumValue(typeof(CipherMode), text6)))
                    {
                        case CipherMode.CBC:
