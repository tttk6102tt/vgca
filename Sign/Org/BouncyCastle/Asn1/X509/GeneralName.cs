...
namespace Sign.Org.BouncyCastle.Asn1.X509
{
    public class GeneralName : Asn1Encodable, IAsn1Choice
    {
        ...
        {
            if (Sign.Org.BouncyCastle.Utilities.Net.IPAddress.IsValidIPv6WithNetmask(ip) || Sign.Org.BouncyCastle.Utilities.Net.IPAddress.IsValidIPv6(ip))
            {
                int num = ip.IndexOf('/');
                if (num < 0)
                {
            ...
            if (Sign.Org.BouncyCastle.Utilities.Net.IPAddress.IsValidIPv4WithNetmask(ip) || Sign.Org.BouncyCastle.Utilities.Net.IPAddress.IsValidIPv4(ip))
            {
                int num2 = ip.IndexOf('/');
                if (num2 < 0)
                {
