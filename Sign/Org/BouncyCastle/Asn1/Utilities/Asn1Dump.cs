...
namespace Sign.Org.BouncyCastle.Asn1.Utilities
{
    public sealed class Asn1Dump
    {
        ...
        private static void AsString(string indent, bool verbose, Asn1Object obj, StringBuilder buf)
        {
            ...
            else if (obj is DerUnknownTag)
            {
                ...
                buf.Append(indent + "Unknown " + ((DerUnknownTag)obj).Tag.ToString("X") + " " + text8 + NewLine);
            }
            ...
            else if (obj is DerExternal)
            {
                ...
                {
                    buf.Append(text9 + "Direct Reference: " + derExternal.DirectReference.Id + NewLine);
                }

                if (derExternal.IndirectReference != null)
                {
                    buf.Append(text9 + "Indirect Reference: " + derExternal.IndirectReference.ToString() + NewLine);
                }

                if (derExternal.DataValueDescriptor != null)
                {
                    AsString(text9, verbose, derExternal.DataValueDescriptor, buf);
                }

                buf.Append(text9 + "Encoding: " + derExternal.Encoding + NewLine);
                AsString(text9, verbose, derExternal.ExternalContent, buf);
            }
            else
            {
