...
namespace Sign.Org.BouncyCastle.Tsp
{
    public class TspUtil
    {
        ...
        public static ICollection GetSignatureTimestamps(SignerInformation signerInfo)
        {
            ...
            {
                foreach (Asn1.Cms.Attribute item in unsignedAttributes.GetAll(PkcsObjectIdentifiers.IdAASignatureTimeStampToken))
                {
                    foreach (Asn1Encodable attrValue in item.AttrValues)
                    {
                        try
                        {
                            TimeStampToken timeStampToken = new TimeStampToken(ContentInfo.GetInstance(attrValue.ToAsn1Object()));
